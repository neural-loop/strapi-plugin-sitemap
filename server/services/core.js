...
const getLanguageLinks = async (page, contentType, defaultURL, excludeDrafts) => {
    const config = await getService('settings')
        .getConfig();
    ...
    await Promise.all(page.localizations.map(async (translation) => {
        const translationEntity = await strapi.query(contentType)
            .findOne({
                where: {
                    $or: [
                        {
...
const createSitemapEntries = async () => {
    ...
    // Collection entries.
    await Promise.all(Object.keys(config.contentTypes)
    ...
    // Custom entries.
    await Promise.all(Object.keys(config.customEntries)
        .map(async (customEntry) => {
            sitemapEntries.push({
                url: customEntry,
...
const writeSitemapFile = (filename, sitemap) => {
    streamToPromise(sitemap)
        .then((sm) => {
            let file = sm.toString()
            const filename = blobFrom(file, 'text/plain');
            const form = new FormData();

            form.append('files', file, "sitemap.txt");
